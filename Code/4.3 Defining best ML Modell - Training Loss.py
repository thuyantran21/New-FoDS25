...
model_dict2 = {name: regressor_map[name]() for name in regressor_map}
...
for target in targets:
    ...
    plt.grid(True)
    plt.tight_layout()
    #plt.savefig(f"../Output/ValidationLoss_{target.replace(' ', '_')}.png")
    #plt.show()
...
for model_name in top_4_ML:
    ...
    if relative_diff <= threshold:
        ...
        print("✅ Training and Validation Loss show small derivations.")
        print("✅ Good generalization. But beginning overfitting risk.")

    else:
        print("⚠️ Significant gap between training and validation.")
        print("⚠️ Possible overfitting or underfitting.")
